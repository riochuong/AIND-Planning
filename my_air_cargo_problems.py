...
class AirCargoProblem(Problem):
            ...
            for cargo in self.cargos:
                for plane in self.planes:
                    for airport in self.airports:
                        precond_pos = [
                                        # cargo need to be at the airport
                                        expr("At({},{})".format(cargo,airport)),
                                        # plane need to be at the airport
                                        expr("At({},{})".format(plane,airport)),
                                      ]
                        precond_neg = []
                        effect_add = [
                                        # cargo now on the plane
            ...
            for cargo in self.cargos:
                for plane in self.planes:
                    for airport in self.airports:
                        ...
                        precond_neg = []
                        effect_add = [
                                        # cargo now on unload at the airport
                                        expr("At({},{})".format(cargo,airport))
    ...
    def h_ignore_preconditions(self, node: Node):
        ...
        # approximate number of goal
        state = decode_state(node.state, self.state_map)
        count = len(self.goal)
        # count number of unsatisfied goal
        #print("goal",self.goal)
        # check how many goals satisfied the proglem
        for st in self.goal:
            if st in state.pos:
                count -= 1
        return count
